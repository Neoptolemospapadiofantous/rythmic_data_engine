#!/usr/bin/env python3
"""
Python Standards Check — enforces python_standards.yaml rules against the codebase.

For each rule with check: regex_absent, scans matching Python files and flags any line
where the pattern is found. For check: manual_review, emits a reminder. Exit 1 on ERROR.

Usage:
  python scripts/python_standards_check.py           # run all checks
  python scripts/python_standards_check.py --json    # JSON output
  python scripts/python_standards_check.py --errors-only  # ERROR/CRITICAL only
"""
import argparse
import fnmatch
import json
...
RULES_PATH = PROJECT_ROOT / "quality_rules" / "python_standards.yaml"
...
def _scan_regex_absent(rule: dict, root: Path) -> list[dict]:
    ...
    scope_str = rule.get("scope", "*.py")
    exclude = rule.get("exclude", [])
    message = rule.get("message", f"Rule {rule_id} violated")
    known_violations = {
        v["file"] for v in rule.get("known_violations", [])
    }
    ...
    for path in files:
        ...
        for line_no, line in enumerate(text.splitlines(), 1):
            if rx.search(line):
                status = "WARN" if rel in known_violations else severity
                findings.append({
                    "rule_id": rule_id,
                    "severity": severity,
...
def run_rules(rules_data: dict, root: Path) -> list[dict]:
    ...
    for rule in rules_data.get("rules", []):
        ...
        if check == "regex_absent":
            ...
            findings.append({
                "rule_id": rule["id"], "severity": "INFO",
                "message": f"[{rule['id']}] {rule.get('name', '')} — manual review required: {rule.get('description', '')}",
...
def main():
    ...
    for f in findings:
        if f["status"] in ("ERROR", "CRITICAL") or f["severity"] in ("ERROR", "CRITICAL"):
            if f["status"] not in ("PASS", "INFO", "SKIP", "WARN"):
